...
public class SubmitLocationLabel extends AppCompatActivity {
    ...
    public void submitButtonPressed(View view){
      ...
      labelList =   myDatabaseHelper.getAllLocationLabels();


      for(int i=0;i<labelList.size();i++) {
          Log.d("PRINTING LOCATION LABELS ", labelList.get(i));
      }

      String test = "Dolphins";

      ArrayList<String> locationFeatures = new ArrayList<String>();

      Cursor labelCursor = myDatabaseHelper.CheckIfLocationLabelExists(test);


      Log.d("labelCursor",labelCursor+" ");

        if(labelCursor.getCount()!=0) {  //If the locationFeatures data is not empty, then add to ArrayList

            while (labelCursor.moveToNext()) {

                locationFeatures.add(labelCursor.getString(1));
        ...
        for(int x=0;x<locationFeatures.size();x++){

            Log.d("IM HERE PRINTING LABEL LOCATIONDATA",locationFeatures.get(x));
        }
